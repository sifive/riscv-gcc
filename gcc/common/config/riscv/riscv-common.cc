...
public:
  ...
  riscv_version_t default_version () const
  {
	...
	if (ver.isa_spec_class == riscv_isa_spec
	    ...
	    || ver.isa_spec_class == ISA_SPEC_CLASS_UNRATIFIED)
	  return ver;
      }
    gcc_unreachable ();
  }
		    ...
		    int minor_version)
{
  ...
  for (const riscv_version_t &ver : itr->second.supported_versions ())
    if (ver.major_version == major_version
	&& ver.minor_version == minor_version)
      return ver.isa_spec_class;
		     ...
		     enum riscv_isa_spec_class *isa_spec_class)
{
  ...
  *minor_version = ver.minor_version;
  if (isa_spec_class)
    *isa_spec_class = ver.isa_spec_class;
}
  ...
  *explicit_version_p = false;
  ...
  if (major == 0 && minor == 0)
  ...
  else
    {
      ...
      bool found = (isa_spec_class != ISA_SPEC_CLASS_UNKNOWN);
